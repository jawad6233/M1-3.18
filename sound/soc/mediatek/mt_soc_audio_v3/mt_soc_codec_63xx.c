...
//Start[Runbo,BruceLai,2016-08-20]Enable system audio speaker.
#if defined(CONFIG_TINTELE_DEVICE_MODEL_M1)
extern void set_system_spk_mode(int spk_en);
#endif
//End[Runbo,BruceLai,2016-08-20]
...
#define SPK_WARM_UP_TIME        (25)	/* unit is ms */
	...
	//End[Runbo,BruceLai,2016-08-20]
		...
		udelay(500);
#endif
	//Start[Runbo,BruceLai,2016-08-20]Disable system audio speaker.
#if defined(CONFIG_TINTELE_DEVICE_MODEL_M1)
	set_system_spk_mode(0);
#else
	AudDrv_GPIO_EXTAMP_Select(false);
#endif
	//End[Runbo,BruceLai,2016-08-20]
...
//Start[Runbo,BruceLai,2016-08-20]Export an function for IntercomMic.
#if defined(CONFIG_TINTELE_DEVICE_MODEL_M1)
volatile int runboMicUsedCount = 0;
#endif
//End[Runbo,BruceLai,2016-08-20]
...
#if defined(CONFIG_TINTELE_DEVICE_MODEL_M1)
				runboMicUsedCount--;
                if(runboMicUsedCount <= 0)
                {    
                    runboMicUsedCount = 0; 
                }    
                else 
                {    
                    return true; //someone is using the MIC ,donot disable the MicBias
                }
#endif
				//End[Runbo,BruceLai,2016-08-20]
				/* "ADC1", main_mic */
...
//Start[Runbo,BruceLai,2016-08-20]Export an function for IntercomMic.
#if defined(CONFIG_TINTELE_DEVICE_MODEL_M1)
void TurnOnIntercomMicPowerACC(bool enable, int audio_mode)
{
	if (audio_mode == 0)
	...
	TurnOnADcPowerACC(AUDIO_ANALOG_DEVICE_IN_ADC1, enable);
}
EXPORT_SYMBOL(TurnOnIntercomMicPowerACC);
#endif
//End[Runbo,BruceLai,2016-08-20]Export an function for IntercomMic.
