...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = GC2755_SENSOR_ID_DT800,        //record sensor id defined in Kd_imgsensor.h
    ...
    .i2c_addr_table = {0x78,0x79, 0xff},//record sensor support all write id addr, only supprt 4must end with 0xff
};
...
//Ink add for a camera sensor can't read ID during boot
//just for dt800
#include <mach/mt_gpio.h>
#include <cust_gpio_usage.h>
//extern int is_camera_sensor_ov5648;  
extern int pinSetIdx_global;
//end
static kal_uint32 return_sensor_id()
{
   // return ((read_cmos_sensor(0xf0) << 8) | read_cmos_sensor(0xf1));
//	if (mt_get_gpio_out(GPIO_CAMERA_CMRST1_PIN)==1)
	if (pinSetIdx_global)
		{
      			return GC2755_SENSOR_ID_DT800;
		}
	else {
		return 0xffffffff;
	}
...
UINT32 GC2755_MIPI_RAW_SensorInit_DT800(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
