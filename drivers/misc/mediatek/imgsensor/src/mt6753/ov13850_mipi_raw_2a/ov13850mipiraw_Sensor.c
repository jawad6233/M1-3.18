...
int ov13850_chip_ver_2a = OV13850_R2A;
//sensor otp
extern void otp_cali(unsigned char writeid);

static imgsensor_info_struct imgsensor_info = {
    .sensor_id = OV13850_SENSOR_ID_2A,        //record sensor id defined in Kd_imgsensor.h
...
//Ink [2015.08.28] Begin
#if ((defined CONFIG_TINTELE_PROJECT_DT800) || (defined CONFIG_TINTELE_PROJECT_DT900))
//#ifdef CONFIG_TINTELE_PROJECT_DT800
    .mirror = IMAGE_H_MIRROR,                //mirrorflip information IMAGE_H_MIRROR, IMAGE_NORMAL , IMAGE_V_MIRROR, IMAGE_HV_MIRROR
#else
    .mirror = IMAGE_V_MIRROR,                //mirrorflip information IMAGE_H_MIRROR, IMAGE_NORMAL , IMAGE_V_MIRROR, IMAGE_HV_MIRROR
#endif
//end
...
kal_uint32 return_sensor_id(void)
{
	...
	if ((read_cmos_sensor(0x302a))==0xb2)
		{
			LOG_INF("----R2A---- \n");
			ov13850_chip_ver_2a = OV13850_R2A;

			//return ((read_cmos_sensor(0x300A) << 8) | read_cmos_sensor(0x300B));
			if (((read_cmos_sensor(0x300A) << 8) | read_cmos_sensor(0x300B)) == 0xD850)
				return 0xD851;
		}
	else
		return 0xffffffff;
...
//	kal_uint16 iGain=gain;
	...
	if (ov13850_chip_ver_2a == OV13850_R1A)
	{
	...
	else if(ov13850_chip_ver_2a == OV13850_R2A)
	{
...
static kal_uint32 open(void)
{
    ...
    if (imgsensor_info.sensor_id != sensor_id)
		...
		{
				LOG_INF("----R1A---- \n");
				ov13850_chip_ver_2a = OV13850_R1A;
                          ...
                          MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {//PIP capture: 24fps for less than 13M, 20fps for 16M,15fps for 20M
        ...
        if (imgsensor.current_fps != imgsensor_info.cap.max_framerate)
            LOG_INF("Warning: current_fps %d fps is not support, so use cap's setting: %d fps!\n",(int)imgsensor.current_fps,(int)imgsensor_info.cap.max_framerate/10);
        ...
        case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
        	  if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {
                ...
                spin_lock(&imgsensor_drv_lock);
        		    ...
        		    if (imgsensor.current_fps != imgsensor_info.cap.max_framerate)
                    LOG_INF("Warning: current_fps %d fps is not support, so use cap's setting: %d fps!\n",(int)framerate,(int) imgsensor_info.cap.max_framerate/10);
...
UINT32 OV13850_MIPI_RAW_SensorInit_2A(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
