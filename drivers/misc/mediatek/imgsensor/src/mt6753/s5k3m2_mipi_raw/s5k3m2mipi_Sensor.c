...
int* get_otp_vcm_data()
{
  ...
  mdelay(10);
  write_cmos_sensor_8(0x0A02,0x1f);
  write_cmos_sensor_8(0x0A00,0x01);
  mdelay(20);
  for(i=0;i<64;i++)
  {
  	otpdata[i] = read_cmos_sensor_8(0x0A04 + i);
  }
  ...
  if (flag1 == 0x10)
  {
   AFcode[0] = (otpdata[21]&0xff)|(otpdata[22]<<8);
   AFcode[1] = (otpdata[23]&0xff)|(otpdata[24]<<8);
   LOG_INF("get_otp_vcm_data,info_flag1 = 0x%02x",flag1);
  }
  else if (flag2 == 0x10)
  {
   AFcode[0] = (otpdata[53]&0xff)|(otpdata[54]<<8);
   AFcode[1] = (otpdata[55]&0xff)|(otpdata[56]<<8);
   LOG_INF("get_otp_vcm_data,info_flag2 = 0x%02x",flag2);
  }
  else
  {
  	LOG_INF("get_otp_vcm_data,AF Invalid!");
  	return NULL;
  }
  LOG_INF("get_otp_vcm_data,Infcode = %d ,Marcode = %d ", AFcode[1],AFcode[0]);
  return AFcode;
}  
*/
...
static void set_mirror_flip(kal_uint8 image_mirror)
{
	...
	LOG_INF("image_mirror = %d, %d ", image_mirror,imgsensor.mirror);
    ...
    switch (image_mirror) {
        ...
        case IMAGE_NORMAL:
        	...
        	write_cmos_sensor(0x602A, 0x0101);
            write_cmos_sensor_8(0x6F12,0x03);   // GB
            break;
        case IMAGE_H_MIRROR:
        	write_cmos_sensor(0x6028, 0x4000);
        	write_cmos_sensor(0x602A, 0x0101);
        ...
        case IMAGE_HV_MIRROR:
        	write_cmos_sensor(0x6028, 0x4000);
        	write_cmos_sensor(0x602A, 0x0101);
            write_cmos_sensor_8(0x6F12,0x00);   // GR
            break;
        default:
...
//  chip_id = read_cmos_sensor(0x0002);
  ...
  write_cmos_sensor(0x602A, 0x0101);
  write_cmos_sensor_8(0x6F12, 0x03);//HV_MIRROR
...
//	otp_lsc_update();
    ...
    LOG_INF("Exit\n");
  //s 2014/12/01, bruce
  } else if ((chip_id == 0xD201)||(chip_id == 0xD101)) {
    LOG_INF("-- sensor_init, chip_id = 0x0%x\n", chip_id);
  write_cmos_sensor(0x6028, 0x4000);
...
// s bruce, 2014/12/01
	...
	LOG_INF( "-- normal_capture_setting, chip_id = 0x0%x\n", chip_id);
write_cmos_sensor_8(0x0100,0x00);
mdelay(200);
write_cmos_sensor(0x6028,0x2000);
write_cmos_sensor(0x602A,0x14F0);
...
{
	LOG_INF( "--pip_capture_setting, chip_id = 0x0%x\n", chip_id);	
	write_cmos_sensor_8(0x0100,0x00);
	mdelay(200);
	write_cmos_sensor(0x6028,0x2000);
	write_cmos_sensor(0x602A,0x14F0);
...
// s bruce 2014/12/01
	...
	LOG_INF("--hs_video_setting. chip_id = 0x0%x\n", chip_id);
write_cmos_sensor_8(0x0100,0x00);
mdelay(200);
	write_cmos_sensor(0x6028,0x2000);
	write_cmos_sensor(0x602A,0x14F0);
...
//s bruce, 2014/12/01
	...
	LOG_INF("--slim_video_setting. chip_id = 0x0%x\n", chip_id);
	write_cmos_sensor_8(0x0100,0x00);
  mdelay(200);
	write_cmos_sensor(0x6028,0x2000);
	write_cmos_sensor(0x602A,0x14F0);
...
static kal_uint32 open(void)
{
	...
	if((chip_id == 0xC001) || (chip_id == 0xD201) || (chip_id == 0xD101))
	{}
	else
    {	chip_id = read_cmos_sensor(0x0002);}
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	LOG_INF("feature_id = %d", (int)feature_id);
	switch (feature_id) {
