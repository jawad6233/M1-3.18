...
#define RUNBO_SENDKEY_HEADSET		1
#if defined CONFIG_ACCDET_EINT
#if(RUNBO_SENDKEY_HEADSET ==1)//add by zhangy 20160903
int accdet_headset_sendkey_irq;
static struct work_struct sendkey_eint_work;
static struct workqueue_struct *sendkey_eint_workqueue = NULL;
struct pinctrl_state *senskey_pins_eint_int;
extern struct input_dev *kpd_input_dev;
static int long_press_time;
static struct timer_list buttons_timer;
static int double_key_flag = 0;
static int double_key = 0;
static int first_inter = 1;
static int second_inter = 1;
unsigned int sendkey_gpiopin;
#endif
#endif
...
#ifdef DEBUG_THREAD
#endif
//Start[Runbo,BruceLai;2016-09-20.]Support Intercom headset/speaker switching.
#if defined (CONFIG_TINTELE_DEVICE_MODEL_M1)
#define HEADSET_MODE                    0
#define SPEAKER_MODE                    1
extern unsigned int intercom_audio_switch(int audio_channel);
#endif
//End[Runbo,BruceLai;2016-09-20.]
...
#ifdef CONFIG_ACCDET_EINT
	//modify by zhangy for plug micbus hight
...
//Start[Runbo,BruceLai;2016-09-20.]Support Intercom headset/speaker switching.
#if defined (CONFIG_TINTELE_DEVICE_MODEL_M1)
		intercom_audio_switch(HEADSET_MODE);
#endif
//End[Runbo,BruceLai;2016-09-20.]
	} else {
/*EINT_PIN_PLUG_OUT*/
...
//Start[Runbo,BruceLai;2016-09-20.]Support Intercom headset/speaker switching.
#if defined (CONFIG_TINTELE_DEVICE_MODEL_M1)
		intercom_audio_switch(SPEAKER_MODE);
#endif
//End[Runbo,BruceLai;2016-09-20.]
	}
...
#if(RUNBO_SENDKEY_HEADSET == 1)//add by zhangy 20160903 for single key in headset
struct of_device_id accdet_headset_sendkey_of_match[] = {
	{ .compatible = "mediatek,mt6735-headset_sendkey", },
};

void report_key_pressed(unsigned int keycode)
{
	printk("[Accdet] %s keycode %d\n", __func__, keycode);
	input_report_key(kpd_accdet_dev, keycode, 1);
	input_report_key(kpd_accdet_dev, keycode, 0);
...
int poc_thread(void *data){
	input_report_key(kpd_input_dev, KEY_POC, 1);
	input_sync(kpd_input_dev);
    while (!gpio_get_value(sendkey_gpiopin))
	{
       msleep(100);    
    }
	input_report_key(kpd_input_dev, KEY_POC, 0);
	input_sync(kpd_input_dev);
	poc_task = NULL;
    return 0;
}
...
static void buttons_timer_function(unsigned long data)
{
	...
	/* for first pulg-in not report_key,after pulg-in report_key*/
	if (first_inter) {	
		second_inter = 0;	
		first_inter = 0;
	}	
	...
	if (double_key_flag)
	{
		...
		report_key_pressed(keycode);
	} else
		double_key_flag = 1;	
}

static bool is_sendkey_long_pressed(void)
{
	int index = 0;
	int count = 25;
	while(count>0)
	{ 
		count --;
		if(gpio_get_value(sendkey_gpiopin))
			return false;
		msleep(80);
	}
	printk("Accdet:index=%d count=%d.\n",index,count);
	return true;
}

void report_key_event(void)
{
	unsigned long timeout = jiffies + 5*HZ/10;
	unsigned int keycode;
	static int key_playpuse = 0;
	static unsigned long timeout_old = 0;
	bool sendkey_long_pressed = is_sendkey_long_pressed();
	if(call_status != 0) {
		printk("Accdet phone is on\n");
		if(sendkey_long_pressed)
			keycode = KEY_ENDCALL;
		else
			keycode = KEY_CALL;
		report_key_pressed(keycode);
		...
		printk("Accdet phone is NOT\n");
		if (key_playpuse){
			if(time_before(jiffies, timeout_old)){
			double_key = 1;	// 还没有超时		
		}
			key_playpuse = 0;
		}
		...
		if(sendkey_long_pressed){
			...
			keycode = KEY_ENDCALL;
			report_key_pressed(keycode);
			printk("Accdet:Run sendkey_long_pressed.\n");
		}
		else if(double_key){
			printk("Accdet:Run double_key.\n");
			//keycode = KEY_NEXTSONG;
			double_key = 0;		
			double_key_flag = double_key;
			//report_key_pressed(keycode);
		}
		else{
			printk("Accdet:Run single_key.\n");
			//keycode = KEY_PLAYPAUSE;
			key_playpuse = 1;
			mod_timer(&buttons_timer, jiffies+7*HZ/10);
		}	
		timeout_old = timeout;
	}    
	//#endif
...
static void sendkey_eint_work_callback(struct work_struct *work)
{
	msleep(80);	
	if (cur_eint_state == EINT_PIN_PLUG_IN) 
	{
	...
	send_sendkey_poc();
	report_key_event();
	ACCDET_DEBUG("[Accdet]accdet_headset_sendkey_irq  !!!!!!\n");
}

static irqreturn_t accdet_headset_sendkey_eint_func(int irq, void *data)
{
	ACCDET_DEBUG("[Accdet]Enter accdet_headset_sendkey_eint_func !!!!!!\n");
	queue_work(sendkey_eint_workqueue, &sendkey_eint_work);
	return IRQ_HANDLED;
}

static inline int accdet_headset_sendkey_eint(struct platform_device *accdet_device)
{
	...
	node = of_find_matching_node(node, accdet_headset_sendkey_of_match);
	if (node) {
		of_property_read_u32_array(node, "debounce", ints, ARRAY_SIZE(ints));
		sendkey_gpiopin = ints[0];
		headsetdebounce = ints[1];
		long_press_time = headsetdebounce;//2000
		gpio_set_debounce(sendkey_gpiopin, headsetdebounce);
		accdet_headset_sendkey_irq = irq_of_parse_and_map(node, 0);
		ret = request_irq(accdet_headset_sendkey_irq, accdet_headset_sendkey_eint_func, IRQF_TRIGGER_FALLING, "HEADSET_SENDKEY-eint", NULL);
		if (ret != 0) {
			ACCDET_ERROR("[Accdet]HEADSET_SENDKEY-eint LINE NOT AVAILABLE\n");
		} else {
			ACCDET_ERROR("[Accdet]accdet set EINT finished, HEADSET_SENDKEY-eint=%d\n",accdet_headset_sendkey_irq);
		}
	} else {
		ACCDET_ERROR("[Accdet]%s can't find compatible node\n", __func__);
	}
	enable_irq(accdet_headset_sendkey_irq);
...
static inline int accdet_setup_eint(struct platform_device *accdet_device)
{
	...
	pins_default = pinctrl_lookup_state(accdet_pinctrl1, "acc_default");
	if (IS_ERR(pins_default)) {
		ret = PTR_ERR(pins_default);
		/*dev_err(&accdet_device->dev, "fwq Cannot find accdet pinctrl default!\n");*/
	}
#if(RUNBO_SENDKEY_HEADSET == 1)//set snedkey as eint pin
	senskey_pins_eint_int = pinctrl_lookup_state(accdet_pinctrl1, "acc_sendkey_state_eint_as_int");
	if (IS_ERR(senskey_pins_eint_int)) {
		ret = PTR_ERR(senskey_pins_eint_int);
		dev_err(&accdet_device->dev, "fwq Cannot find accdet pinctrl senskey_pins_eint_int!\n");
		return ret;
	}
	pinctrl_select_state(accdet_pinctrl1, senskey_pins_eint_int);
#endif

	pins_eint_int = pinctrl_lookup_state(accdet_pinctrl1, "acc_state_eint_as_int");
...
int mt_accdet_probe(struct platform_device *dev)
{
	...
	__set_bit(KEY_POC, kpd_accdet_dev->keybit);
	...
	if (g_accdet_first == 1) {
		...
		accdet_eint_workqueue = create_singlethread_workqueue("accdet_eint");
		INIT_WORK(&accdet_eint_work, accdet_eint_work_callback);
#if (RUNBO_SENDKEY_HEADSET == 1)
		sendkey_eint_workqueue = create_singlethread_workqueue("sendkey_eint");
		INIT_WORK(&sendkey_eint_work, sendkey_eint_work_callback);
#endif
		accdet_setup_eint(dev);
#if (RUNBO_SENDKEY_HEADSET == 1)
		accdet_headset_sendkey_eint(dev);
#endif
#endif
		g_accdet_first = 0;
	}
...
void mt_accdet_remove(void)
{
	...
	//destroy_workqueue(sendkey_eint_workqueue);
