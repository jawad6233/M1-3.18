...
//#add by zhangy 20160810 for add pinctl for mag
struct pinctrl *magpinctrl1;
struct pinctrl_state *mag_rst_output0, *mag_rst_output1;
int mag_get_gpio_info(struct platform_device *pdev)
{
	int ret;
	magpinctrl1 = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(magpinctrl1)) {
		ret = PTR_ERR(magpinctrl1);
		dev_err(&pdev->dev, "fwq Cannot find touch pinctrl1!\n");
		return ret;
	}
	mag_rst_output0 = pinctrl_lookup_state(magpinctrl1, "mag_pin_rest0");
	if (IS_ERR(mag_rst_output0)) {
		ret = PTR_ERR(mag_rst_output0);
		dev_err(&pdev->dev, "fwq Cannot findmag_pin_rest0 %d!\n", ret);
				return ret;
	}
	mag_rst_output1 = pinctrl_lookup_state(magpinctrl1, "mag_pin_rest1");
	if (IS_ERR(mag_rst_output1)) {
		ret = PTR_ERR(mag_rst_output1);
		dev_err(&pdev->dev, "fwq Cannot find  pinctrl mag_pin_rest0!\n");
		return ret;
	}
	return 1;
}

int mag_set_reset_pin(int level)
{
	if (level)
		pinctrl_select_state(magpinctrl1, mag_rst_output1);		
	else
		pinctrl_select_state(magpinctrl1, mag_rst_output0);
	return 1;
}
//#endi
