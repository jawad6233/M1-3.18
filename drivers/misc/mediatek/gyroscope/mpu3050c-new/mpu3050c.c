...
static int mpu3000_local_init(struct platform_device *pdev);
       ...
       unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		case GYROSCOPE_IOCTL_SMT_DATA:
			...
			GYRO_LOG("gyroscope read data from kernel OK: SMTdata[0]:%d, copied packet:%zu!\n", SMTdata[0],
...
static int mpu3000_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	int err = 0;
	GYRO_FUN();
printk("00000000000555555555555555555000000\n");
...
printk("000000666666666666666666000000000\n");
	obj->hw = hw;
	...
	/*------------------------mpu3000 attribute file for debug--------------------------------------*/

	if((err = mpu3000_create_attr(&mpu3000_init_info.platform_diver_addr->driver)))
	{
		GYRO_ERR("create attribute err = %d\n", err);
...
static int mpu3000_local_init(struct platform_device *pdev) 
{
    ...
    //GYRO_FUN();
    MPU3000_power(hw, 1);
	printk("00000002222222222222200000000\n");
    if (i2c_add_driver(&mpu3000_i2c_driver))
    {
        GYRO_ERR("add driver error\n");
        return -1;
    }
    printk("00000333333333333333333330000000000\n");
