...
* This work is based on previous job from Matteo Dameno
*
...
*
* To use fifo, use corresponding fifo_mode sysfs file and fifo_enable sysfs
...
*/
/******************************************************************************
 Revision 0.0.2 2014/Apr/04: 2nd draft
******************************************************************************/
...
// #include <linux/module.h>	// ES: some kernel need it
...
#define LPS25_DEV_NAME_PRS 		"lps25h"
...
static const struct lps25_prs_platform_data default_lps25_pdata = {
        //add by cxw 2015-12-23 ; change poll time
	.poll_interval = 20,
	//end
...
static void lps25_prs_device_power_off(struct lps25_prs_data *prs)
{
	...
	if (prs->pdata->power_off)
		prs->pdata->power_off();
#if 0
    #if ((!defined CONFIG_TINTELE_PROJECT_DT600) || (!defined CONFIG_TINTELE_PROJECT_DT600S))
	//add by  2015-12-15;  shut down INT_GPIO
	mt_set_gpio_mode(203 | 0x80000000, GPIO_MODE_GPIO);
    mt_set_gpio_dir(203 | 0x80000000, GPIO_DIR_OUT);
    mt_set_gpio_out(203 | 0x80000000, 0);
	//end
	#else //FOR DT600  xiaopei 
    mt_set_gpio_mode(14 | 0x80000000, GPIO_MODE_GPIO);
    mt_set_gpio_dir(14 | 0x80000000, GPIO_DIR_OUT);
    mt_set_gpio_out(14 | 0x80000000, 0);  
	#endif 
	#endif
	prs->hw_initialized = 0;
}
						...
						struct outputdata *out)
{
	...
	//add by cxw 2015-12-15; for correction pressure data, minus 8hpa
	//out->press = pressure;
	out->press = pressure - (8 * 4096);
	//end
...
static struct device_attribute attributes[] = {
	...
	__ATTR(enable_autozero, 0555, NULL, attr_set_autozero),	
	__ATTR(compensation_param, 0444, attr_get_compensation_param, NULL),
	__ATTR(enable_fifo, 0555, NULL, attr_set_fifo),
	__ATTR(num_samples_fifo, 0555, NULL, attr_set_samples_fifo),
	__ATTR(fifo_mode, 0664, NULL, attr_fifo_mode),
#if DEBUG
	__ATTR(reg_value, 0664, attr_reg_get, attr_reg_set),
	__ATTR(reg_addr, 0555, NULL, attr_addr_set),
#endif
};
...
static int lps25_prs_input_init(struct lps25_prs_data *prs)
{
	...
	prs->input_dev_pres->name = LPS25_DEV_NAME_PRS;
...
struct lps25_prs_data *obj_i2c_data;
static int lps25h_open(struct inode *inode, struct file *file)
{
	...
	buf[0] =  0x0F;
	lps25_prs_i2c_read(obj_i2c_data, buf, 1);
	if(buf[0] != 0xBD)
		return -EINVAL;
...
static long lps25h_unlocked_ioctl(struct file *file, unsigned int cmd,
	unsigned long arg)
{
	struct lps25_prs_data *obj = (struct lps25_prs_data *)file->private_data;
	...
	case BAROMETER_GET_PRESS_DATA:
		...
		err = lps25h_i2c_read_block(client, I2C_AUTO_INCREMENT|PRESS_OUT_XL, date, 0x02);
dat=date[1]<<8|date[0];
		printk("LPS2H dat=%x\n",dat);
	...
	case BAROMETER_GET_TEMP_DATA:
		...
		printk("LPS2H BAROMETER_GET_PRESS_DATA\n");
		//err = lps25h_i2c_read_block(client, I2C_AUTO_INCREMENT|TEMP_OUT_L, date, 0x02);
//dat=date[1]<<8|date[0];
		lps25_prs_get_presstemp_data(obj_i2c_data,&out);
		
buf[0] =  0x0F;
	err = lps25_prs_i2c_read(obj_i2c_data, buf, 1);
		printk("LPS25 0Fdregister=%x\n",buf[0] );

	buf[0] =  0x10;
	err = lps25_prs_i2c_read(obj_i2c_data, buf, 1);
	printk("LPS25 10dregister=%x\n",buf[0] );
	buf[0] =  0x20;
	err = lps25_prs_i2c_read(obj_i2c_data, buf, 1);
	printk("LPS25 20dregister=%x\n",buf[0] );
	buf[0] =  0x21;
	err = lps25_prs_i2c_read(obj_i2c_data, buf, 1);
	printk("LPS25 21dregister=%x\n",buf[0] );
	buf[0] =  0x2E;
	err = lps25_prs_i2c_read(obj_i2c_data, buf, 1);
	printk("LPS25 2Edregister=%x\n",buf[0] );
		//printk("LPS2H temp=%x\n",dat);
		if (copy_to_user(data, &out.temperature, 16)) {
			err = -EFAULT;
			break;
		}
	break;
...
static int lps25_init_client(struct lps25_prs_data *prs)
{
	int err = 0;
	//u8 data = 0;
	u8 buf[2] = {0};
	...
	buf[0] =  0x40; // FIFO mean mode enabled
	buf[1] = CTRL_REG2;
	err = lps25_prs_i2c_write(prs, buf, 1);
	if (err < 0) {
	...
	buf[0] =  0xC4; // activity mode, 25Hz, BDU enabled
	buf[1] = CTRL_REG1;
	err = lps25_prs_i2c_write(prs, buf, 1);
	if (err < 0) {
...
#if ZYTEST
lps25_init_client(prs);
obj_i2c_data = prs;
	err = misc_register(&lps25h_device);
		if (err) {
			printk("misc device register failed, err = %d\n", err);
...
static int lps25_prs_remove(struct i2c_client *client)
{
...
#if 0
static struct i2c_board_info __initdata lps25h_i2c_devices[] = {
	{
		I2C_BOARD_INFO( LPS25_PRS_DEV_NAME, LPS25_PRS_I2C_SAD_L),
	},
};
#endif
...
//modify by zhnagy use device tree
	const char *name = "mediatek,lps25h";
hw =   get_baro_dts_func(name, hw);
	//if (!hw)
		//BAR_ERR("get cust_baro dts info fail\n");
	//i2c_register_board_info(2, lps25h_i2c_devices, ARRAY_SIZE(lps25h_i2c_devices));
#if DEBUG
