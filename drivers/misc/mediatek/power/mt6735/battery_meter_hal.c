...
/* current compensate */
	...
	bm_print(BM_LOG_FULL, "[fgauge_read_current]line:415 ori current=%d\n", dvalue);

	dvalue = use_chip_trim_value(dvalue);

	bm_print(BM_LOG_FULL, "[fgauge_read_current] line419 final current=%d (ratio=%d)\n", dvalue,
...
/* current compensate */
	...
	dvalue = ((dvalue * batt_meter_cust_data.car_tune_value) / 100);
	bm_print(BM_LOG_FULL, "[fgauge_read_IM_current]line:501 ori current=%d\n", dvalue);
...
/* Auto adjust value */
	...
	dvalue_CAR = ((dvalue_CAR * batt_meter_cust_data.car_tune_value) / 100);
		bm_print(BM_LOG_FULL, "[fgauge_read_columb_internal]line:791 new CAR=%d\n", dvalue_CAR);
