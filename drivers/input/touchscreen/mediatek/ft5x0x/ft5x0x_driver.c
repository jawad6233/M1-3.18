...
static int tpd_touchinfo(struct touch_info *cinfo, struct touch_info *pinfo)
{
	...
	printk(" cinfo->x= %d, cinfo->y%d\n", cinfo->x[0],  cinfo->y[0]);
	printk("TPD_RES_x=%d TPD_RES_Y=%d\n",(int)TPD_RES_X,(int)TPD_RES_Y);
#ifdef CONFIG_TPD_HAVE_CALIBRATION
	for (i = 0; i < cinfo->count; i++) {
...
static int tpd_irq_registration(void)
{
	...
	if (node) {
		...
		ret = request_irq(touch_irq,
				  (irq_handler_t)tpd_eint_interrupt_handler,IRQF_TRIGGER_RISING,
