...
/*zougx add for SOS key,init the KPCOL2 mode 2016-8-26 start*/
static void kpclo2_gpio_set(GPIO_PIN pin, GPIO_MODE mode, GPIO_DIR dir,
...
/*zougx add for SOS key,init the KPCOL2 mode 2016-8-26 end*/
...
static int kpd_pdrv_probe(struct platform_device *pdev)
{
	...
	kpd_info("%s Done\n", __func__);
	/*zougx add for SOS key,init the KPCOL2 mode 2016-8-26 start*/
	kpclo2_gpio_set(0x80000000 | GPIO86, 1, 0, 0, 1, 1, 1);//the kpcol2 is gpio86
	/*zougx add for SOS key,init the KPCOL2 mode 2016-8-26 end*/
	return 0;
}
