...
//Ink 2016.07.31
	if (g_fg_is_charging == KAL_TRUE )
		...
		ret_value = 0- battery_meter_get_battery_current();
...
#include "../../misc/mediatek/leds/mt6735/leds_sw.h"
...
//Ink add to fix bug: power_off_charging leds do not work
	if (1){ //boot_mode = power_off_charging_mode
		if (BMT_status.charger_exist == KAL_TRUE){
			if (BMT_status.UI_SOC <90)
				mt_brightness_set_pmic(1, 255, 0);
			else
				mt_brightness_set_pmic(2, 255, 0);
